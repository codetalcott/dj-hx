...
observed rather than reasoned about. Same seven scenarios as hx-flask. Opt-in:
...
from tests.conftest import CONTACTS_JSON  # noqa: E402


@pytest.fixture
def live(live_server, transactional_db, monkeypatch):
    """The contact app on a live server: data loaded, sleeps off, archiver idle."""
...
def test_lazy_count_then_row_delete_fades_out_and_flashes_in_place(page, live):
    ...
    expect(page.locator("#messages")).to_contain_text("Deleted Contact!")
...
def test_bulk_delete_rerenders_rows_and_the_count_hears_the_event(page, live):
    ...
    expect(page.locator("#messages")).to_contain_text("Deleted Contacts!")
...
def test_new_contact_success_redirects_with_the_flash(page, live):
    ...
    expect(page.locator("#messages")).to_contain_text("Created New Contact!")
