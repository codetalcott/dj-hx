"""The linter (vendored verbatim from hx-flask): HCON port, each rule, and the contact app passing clean."""

import pytest

from dj_hx import hx_vocab as V  # noqa: N812
from dj_hx.hxlint import (
    hcon_parse,
    hcon_split,
    lint_html,
    lint_paths,
...
from dj_hx.templates import lint_template_source
...
def test_the_contact_app_templates_lint_clean(capsys):
    """Through hxlint's own Jinja-agnostic path and through the Django stripper."""
    templates = ROOT / "example" / "contacts" / "templates"
    assert lint_paths([str(templates)]) == 0
    out = capsys.readouterr().out
    assert "0 errors" in out and "[warning]" not in out and "[error]" not in out
    for path in templates.glob("*.html"):
        findings = [f for f in lint_template_source(path.read_text(), file=path.name) if f.severity != "info"]
        assert findings == [], path.name


def test_the_lint_hook_is_wired_into_the_test_client(lib, client):
    from dj_hx import HxLintError
